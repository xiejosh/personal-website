...
- `AsciiBackground.tsx` — canvas ASCII background rendered on **every page** (`showCompanies={false}` on non-home pages hides the company shapes but keeps everything else): a grid of faint `#` characters; a moving cursor streams expanding white `~` ring outlines along its path (nothing while stationary); a lone ambient white `~` "ping" ring expands from a random point every ~6–14s; rocket-emoji-style ships (`#` cells, red nose/fins, blue porthole) fly across periodically with `-`/`.` fire trails and steer away from the cursor; company logos are drawn as colored `.` cells along hand-plotted outlines (`CompanyShape` points + segments keyed by `company`), with drift/rotation-wobble/cursor-repulsion animation and cursor hover detection reported via `onCompanyHover`. The home page passes `activeCompany` down to highlight the hovered company's shape. **The `company` strings here must match the `experience` array in `page.tsx`.** The `asteroids` prop (contact page) adds small gray `#`/`o`/`.` rocks tumbling slowly leftward that wrap around the edges — clicking one shatters it into shard/spark debris with an orange shockwave (suppressing the normal click ripple) and a replacement drifts in from the right a few seconds later; the component also exposes a `forwardRef` handle (`AsciiBackgroundHandle`) with `ping(clientX, clientY)` to drop a sonar ring at viewport coords — the contact page pings behind each asteroid card on hover/focus. The contact page (md+) is an asteroid-belt layout: contacts as floating cratered-blob asteroids (emails copy to clipboard with a `✓ copied...` status line under the header; GitHub/LinkedIn open links), with the same overlay format as projects (header top-left, Terminal bottom-left) and a stacked-card fallback on mobile.
- `Terminal.tsx` — interactive terminal on every page (home, about, projects, contact): visitors type commands (`help`, `whoami`, `ls`, `cd <page>` navigates via router, `experience`/`cat experience.log`, `clear`, `sudo` easter egg) with arrow-key command history; typing a printable character anywhere on the page (outside inputs) focuses it and captures the keystroke. Content like the experience lines is duplicated here — keep in sync with `page.tsx`.
- `NowPlaying.tsx` — polls the Last.fm API (public key inline) every 30s to show the current/last Spotify track.
- `Visitors.tsx` — visitor counting via Supabase REST (publishable key inline; table `daily_visits`, increments through a `record_visit()` security-definer RPC, reads via read-only RLS): `VisitTracker` (in `layout.tsx`, records once per session via sessionStorage, skips localhost) and `VisitorChart` (home page, two-column ASCII `#` bar chart of the past 6 UTC days under `experience.log`, kept short so the home page never scrolls; renders nothing if the fetch fails).
