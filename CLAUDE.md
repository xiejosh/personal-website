...
- `Terminal.tsx` — interactive terminal on the home, about, and projects pages: visitors type commands (`help`, `whoami`, `ls`, `cd <page>` navigates via router, `experience`/`cat experience.log`, `clear`, `sudo` easter egg) with arrow-key command history; typing a printable character anywhere on the page (outside inputs) focuses it and captures the keystroke. Content like the experience lines is duplicated here — keep in sync with `page.tsx`.
